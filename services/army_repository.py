...
class ArmyRepository():
  ...
  def _read_file(self) -> FileReadStatus:
    ...
    try:
      ...
      messages.append(f"Failed to decode JSON from the file {self.filepath}. The file may be malformed")
      success = False
  ...
  def _write_file(self, data) -> FileWriteStatus:
    ...
    try:
      ...
      messages.append(f"Permission denied when trying to write to '{self.filepath}'.")
      success = False
    ...
    try:
      ...
      for unit in army_data["units"]:
        ...
        if not isinstance(unit_quantity, int):
          messages.append(f"Invalid unit quantity: {unit_quantity}, must be an integer")
          is_valid = False
        if not unit_quantity > 0:
          messages.append(f"Invalid unit quantity: {unit_quantity}, must be greater than 0")
          is_valid = False
  ...
  def save(self, army: Army) -> ReturnArmyStatus:
    ...
    if not read_status["success"]:
      return {"success": read_status["success"], "army": army, "messages": messages}
  ...
  def load(self, name: str, game_data: GameData) -> ReturnArmyStatus:
    ...
    if not file_status["success"]:
      return {"success": file_status["success"], "army": army, "messages": messages}
