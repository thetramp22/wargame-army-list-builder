...
class ArmyBuilder():
  ...
  def show_army_summary(self, army: Army | None):
    if army == None:
      return
  ...
  def save_army_to_file(self, army: Army | None):
    ...
    if not os.path.exists(self.filepath):
      print(f"Armies file not found, creating new file at: {self.filepath}")
    ...
    with open(self.filepath, 'w') as file:
      ...
      print(f"{army.name} saved successfully to {self.filepath}")
  ...
  def load_army_from_file(self, army_name: str):
    ...
    if army_name in file_data["armies"]:
      ...
      print(f"{army_to_load.name} loaded from {self.filepath}")
    else:
      print(f"No army named {army_name} found in {self.filepath}")
    return army_to_load
