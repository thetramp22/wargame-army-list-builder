...
class ArmyBuilder():
  def __init__(self, game_data: GameData, filepath: str):
  ...
  def remove_unit_from_army(self, unit_id: str, army: Army, quantity: int= 1):
    ...
    if matching_unit is None:
      ...
      messages.append(f"{army.name} only has {matching_unit.quantity} {matching_unit.unit.name} units, all were removed")      
